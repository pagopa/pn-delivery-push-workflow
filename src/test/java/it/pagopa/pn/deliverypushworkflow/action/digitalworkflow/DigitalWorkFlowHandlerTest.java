package it.pagopa.pn.deliverypushworkflow.action.digitalworkflow;
...
import it.pagopa.pn.deliverypushworkflow.dto.ext.externalchannel.*;
...
class DigitalWorkFlowHandlerTest {
    ...
    @Mock
    private PnDeliveryPushWorkflowConfigs.ExternalChannel externalChannel;
    @Mock
    ...
    void nextWorkFlowAction_0_NotGeneral_WithAddress() {
        ...
        when(pnDeliveryPushConfigs.getExternalChannel()).thenReturn(Mockito.mock(PnDeliveryPushWorkflowConfigs.ExternalChannel.class));
    ...
    void handleExternalChannelResponseProgressRetryable_008_010() {
        ...
        when(notificationService.getNotificationByIun(Mockito.anyString()))
                .thenReturn(notification);
        PnDeliveryPushWorkflowConfigs.ExternalChannel externalChannelConfig = Mockito.mock(PnDeliveryPushWorkflowConfigs.ExternalChannel.class);
        ...
        when(auditLogEvent.generateWarning(Mockito.any(), (EventCodeInt) Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(auditLogEvent);
        ...
        when( auditLogService.buildAuditLogEvent(Mockito.anyString(), Mockito.anyInt(), Mockito.eq(PnAuditLogEventType.AUD_DD_RECEIVE), Mockito.anyString(), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(auditLogEvent);
        ...
        Mockito.verify( auditLogEvent).generateWarning(Mockito.any(), (EventCodeInt) Mockito.any(), Mockito.any(), Mockito.any());
        ...
        when(digitalWorkFlowUtils.getPreviousTimelineProgress(Mockito.any(), Mockito.anyInt(), Mockito.anyInt(), Mockito.any(Boolean.class), Mockito.any()))
    ...
    void retryWorkFlowAction() {
        ...
        Mockito.when(pnDeliveryPushConfigs.getExternalChannel()).thenReturn(Mockito.mock(PnDeliveryPushWorkflowConfigs.ExternalChannel.class));
    ...
    void retryWorkFlowAction3() {
        ...
        when(digitalWorkFlowUtils.getTimelineElement(Mockito.anyString(), Mockito.eq(sourceTimelineId))).thenReturn(Optional.of(element));
        ...
        Assertions.assertEquals(TimelineElementCategoryInt.SEND_DIGITAL_DOMICILE, element.getCategory());
    }
