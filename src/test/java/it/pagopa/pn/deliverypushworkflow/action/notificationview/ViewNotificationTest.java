...
class ViewNotificationTest {
    ...
    void setup() {
        ...
        Mockito.lenient().when(safeStorageService.getFile(Mockito.anyString(), eq(true), eq(false)))
    ...
    void startVewNotificationProcess() {
        ...
        //WHEN
        viewNotification.startVewNotificationProcess(notification, recipient, notificationViewedInt, false).block();
    ...
    @Test
    @ExtendWith(MockitoExtension.class)
    void startVewNotificationProcessWithDelegateFallbackOnMandatesInfo() {
        //GIVEN
        NotificationRecipientInt recipient = NotificationRecipientTestBuilder.builder().build();
        NotificationInt notification = NotificationTestBuilder.builder()
                .withNotificationRecipient(recipient)
        ...
        BaseRecipientDtoInt delegateDtoInt = BaseRecipientDtoInt.builder()
                .denomination("testDenominationFallback")
                .build();

        when(confidentialInformationService.getDelegateInformationByMandateId(delegateInfo.getMandateId(), delegateInfo.getDelegateType()))
                .thenReturn(Mono.just(delegateDtoInt));
        ...
        DelegateInfoInt delegateInfoEnriched = delegateInfo.toBuilder()
                .taxId(baseRecipientDtoInt.getTaxId())
                .denomination(delegateDtoInt.getDenomination())
                .build();
    ...
    void startVewNotificationProcessBlockedWhenAttachmentIsMissing() {
        ...
        //WHEN
        Boolean result = viewNotification.startVewNotificationProcess(notification, recipient, notificationViewedInt,false).block();
    ...
    @Test
    @ExtendWith(MockitoExtension.class)
    void startVewNotificationProcessNonBlockedWhenAttachmentIsMissingButFeatureFlagDisabled() {
        //GIVEN
        String legalFactsId = "legalFactsId";
        when(legalFactStore.sendCreationRequestForNotificationViewedLegalFact(Mockito.any(NotificationInt.class), Mockito.any(NotificationRecipientInt.class), Mockito.isNull(), Mockito.any(Instant.class)))
                .thenReturn(Mono.just(legalFactsId));
