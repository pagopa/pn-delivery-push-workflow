...
public class PaperChannelApiConfigurator {
    @Bean
    @Primary
    public PaperMessagesApi paperMessagesApi(@Qualifier("withOffsetDateTimeFormatter") RestTemplate restTemplate, PnDeliveryPushWorkflowConfigs cfg){
    ...
    @Bean
    @Primary
    public CheckAddressApi checkAddressApi(@Qualifier("withOffsetDateTimeFormatter") RestTemplate restTemplate, PnDeliveryPushWorkflowConfigs cfg){
        ApiClient apiClient = new ApiClient(restTemplate);
        apiClient.setBasePath(cfg.getPaperChannelBaseUrl());
