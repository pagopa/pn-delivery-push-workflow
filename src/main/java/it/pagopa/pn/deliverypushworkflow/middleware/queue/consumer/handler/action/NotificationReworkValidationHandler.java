...
@Component
@CustomLog
public class NotificationReworkValidationHandler extends AbstractActionEventHandler {
    private final ReworkValidationHandler reworkValidationHandler;

    public NotificationReworkValidationHandler(TimelineUtils timelineUtils, ReworkValidationHandler reworkValidationHandler) {
        super(timelineUtils);
        this.reworkValidationHandler = reworkValidationHandler;
    }
    ...
    public void handle(Action action, MessageHeaders headers) {
        ...
        MDCUtils.addMDCToContextAndExecute(Mono.just(action)
                    .flatMap(reworkValidationHandler::handleNotificationRework)
