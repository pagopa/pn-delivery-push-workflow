...
import it.pagopa.pn.deliverypushworkflow.util.NationalRegistriesMessageUtil;
...
public class NationalRegistriesClientImpl extends CommonBaseClient implements NationalRegistriesClient {
    ...
    public void sendRequestForGetDigitalAddress(String taxId, String recipientType, String correlationId, Instant notificationSentAt) {
        ...
        AddressRequestBodyFilterDto addressRequestBodyFilter = new AddressRequestBodyFilterDto()
                .taxId(taxId)
                .correlationId(correlationId)
                .referenceRequestDate(notificationSentAt)
                .domicileType(AddressRequestBodyFilterDto.DomicileTypeEnum.DIGITAL);
        
        MDCUtils.addMDCToContextAndExecute(
                addressApi.getAddresses(recipientType, new AddressRequestBodyDto().filter(addressRequestBodyFilter), PN_NATIONAL_REGISTRIES_CX_ID_VALUE)
    ...
    @Override
    public CheckTaxIdOKDto checkTaxId(String taxId) {
        log.logInvokingExternalService(CLIENT_NAME, CHECK_TAX_ID);

        CheckTaxIdRequestBodyDto checkTaxIdRequestBody = new CheckTaxIdRequestBodyDto()
                .filter(
                    new CheckTaxIdRequestBodyFilterDto()
                            .taxId(taxId)
                );
    ...
    @Override
    public List<NationalRegistriesResponse> sendRequestForGetPhysicalAddresses(PhysicalAddressesRequestBodyDto physicalAddressesRequestBody) {
